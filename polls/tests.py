...
def create_question(question_text, days):
    """
    Create question with the given 'question_text' and publish the 
    given number of 'days' offset to now (negative for those questions published in the past
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_past_question(self):
        """
        Question with a pub_date in the past are displeyed on the index page
        """
    ...
    def test_future_question(self):
        """
        Question with a pub_date in the future aren't displeyed on the index page
        """
        create_question(question_text="Future question.", days=30)
        response = self.client.get(reverse('polls:index'))
        self.assertContains(response, "No polls are avaliable")
        self.assertQuerryseyEqual(response.context['latest_question_list'],[])
